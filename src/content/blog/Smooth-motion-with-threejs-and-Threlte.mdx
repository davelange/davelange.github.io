...
description: >
  Creating 3D scenes in the browser is a rewarding and fun task, but it's also
  challenging. Let's take a look at creating movement in Threlte and three.js
publishedAt: 2024-05-01T23:00:00.000Z
canonical: "https://subvisual.com/blog"
---

# Smooth motion with three.js and Threlte

_This article was originally written for [Subvisual](https://subvisual.com/blog)._

Creating 3D scenes in the browser is a rewarding and fun task, but it's also challenging. There are plenty of things to consider - lights, cameras, materials, etc - just to setup a decent looking static scene. And then we have to decide how to add some movement to our scene.

![full_demo](https://hackmd.io/_uploads/SJISJU0WC.gif)
...
There's a lot going on here, so I'll try to cover the main topics, especially regarding motion. You can check out the [repo on Github](https://github.com/davelange/cubed/) for the full code.
...
Another big part of this is the physics engine. Basically this simulates a physical world where objects act like physical objects - they have mass, friction, etc, and are affected by gravity and collisions. I used [Rapier](https://rapier.rs/) - a fast physics engine written in Rust which can be easily used both in React and Svelte projects through wrapper libraries.
...
Let's start with the simpler parts of the project - in this scene the camera and ligths are nothing special.
...
<World gravity={[0, 0, 0]}>
  <T.PerspectiveCamera
    ...
    makeDefault
  />
  <T.DirectionalLight color="#f8f1ed" position={[0, 0, 15]} intensity={0.5} />
  <T.PointLight color="#f8f1ed" intensity={2} position={[-5, 10, 0]} />

  <Debug />
</World>
```

`World` is a component provided by `@threlte/rapier` that creates the phyisics world. Note that we're setting the gravity to 0, as we don't want our objects to fall.
...
![1cube](https://hackmd.io/_uploads/HyQVeICWC.png)
...
![gif_1](https://hackmd.io/_uploads/S1ot9S6WR.gif)

## Setting boundaries

If all goes well you should now see the cube happily drifting away into the void. The `linearVelocity` property defines the movement velocity on each axis, and you can tweak the `angularVelocity` to make it spin. As you've probably guessed, the `Collider` component will handle collision detections, but we'll get there in a moment. But first, how do we prevent the Cube from disappearing into space?
...
Each limit in our box can be achived using a [PlaneGeometry](https://threejs.org/docs/index.html?q=plane#api/en/geometries/PlaneGeometry). The following code would get you the front and back limits, and the remaining ones need only the appropriate position and rotation values:
...
![regular_physics](https://hackmd.io/_uploads/B1QMz8T-A.gif)
...
![limits](https://hackmd.io/_uploads/SyIwtIabA.gif)
...
- We use the provided `useTask` hook to progress our animation on each frame. On every run, we increment the rigid body translation by a fraction of the animation's total force. When the animation `progress` is equal to it's `duration` we set `animation` back to undefined, as it is complete.

That leaves us with something like this:

![basic_anim](https://hackmd.io/_uploads/HJyf2iR-R.gif)
...
<T.Group
  position={[0, 0, 0]}
  rotation={[degToRad($mouse.y / 100), degToRad($mouse.x / 100), 0]}
>
  <T.Mesh>
    <Text3DGeometry
...
![text](https://hackmd.io/_uploads/SJiGozZzR.gif)
